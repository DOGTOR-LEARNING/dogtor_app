...
class _FriendProfilePageState extends State<FriendProfilePage> {
        ...
        if (data['success'] == true) {
          setState(() {
            ...
            if (data.containsKey('current_streak')) {
              learningStats['current_streak_days'] =
            ...
            if (data.containsKey('learning_days') &&
                data['learning_days'] != null) {
  ...
  Future<void> _fetchOnlineStatus() async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
            'https://superb-backend-1041765261654.asia-east1.run.app/online/status/$userId'),
        headers: {'Accept': 'application/json; charset=utf-8'},
      );
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => BattlePreparePage(
          ...
          opponentName:
              widget.friend['name'] ?? widget.friend['nickname'] ?? '未知用戶',
  ...
  Future<void> _fetchLearningStats() async {
    ...
    try {
      ...
      if (weeklyStatsResponse.statusCode == 200) {
        ...
        // 獲取今日完成的關卡數
        final today =
            DateTime.now().toString().split(' ')[0]; // 獲取當前日期 (YYYY-MM-DD)
  ...
  Future<void> _sendLearningReminder() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse(
            'https://superb-backend-1041765261654.asia-east1.run.app/notifications/send_learning_reminder'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode({
          'user_id': userId,
              ...
              child: Column(
                children: [
                    ...
                    children: [
                      Container(
                        ...
                        child: CircleAvatar(
                          ...
                          backgroundImage: widget.friend['photo_url'] != null &&
                                  widget.friend['photo_url'].isNotEmpty
                              ? NetworkImage(widget.friend['photo_url'])
                              : null,
                          child: widget.friend['photo_url'] == null ||
                                  widget.friend['photo_url'].isEmpty
                              ? Text(
                                  (widget.friend['name'] ??
                  ...
                  if (widget.friend['nickname'] != null &&
                      widget.friend['nickname'].toString().isNotEmpty)
                    Container(
                  ...
                  if (!isLoading && learningStats['max_streak_days'] > 0)
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color:
                              FriendProfilePage.progressGreen.withOpacity(0.3),
                          width: 1,
                        ),
                      ),
                      child: Row(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (widget.friend['email'] != null &&
                      widget.friend['email'].toString().isNotEmpty)
                    _buildProfileCard(
                      icon: Icons.email,
                      title: '電子郵件',
                      content: widget.friend['email'].toString(),
                    ),
                  if (widget.friend['year_grade'] != null &&
                      widget.friend['year_grade'].toString().isNotEmpty)
                    _buildProfileCard(
                      icon: Icons.school,
                      title: '年級',
                      content: widget.friend['year_grade'].toString(),
                    ),
                  if (widget.friend['introduction'] != null &&
                      widget.friend['introduction'].toString().isNotEmpty)
                    _buildProfileCard(
                      icon: Icons.person,
                      title: '自我介紹',
  ...
  Widget _buildLearningStreakCard() {
    ...
    final int todayCompletedLevels =
        learningStats['today_completed_levels'] ?? 0;
                  ...
                  Container(
                    ...
                    child: Center(
                      child: Text(
                        ...
                        style: TextStyle(
                          color:
                              hasLearned ? Colors.white : Colors.grey.shade700,
