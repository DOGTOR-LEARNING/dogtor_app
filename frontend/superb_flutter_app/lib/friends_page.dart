...
class _FriendsPageState extends State<FriendsPage>
  ...
  Future<void> _loadFriends() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
            'https://superb-backend-1041765261654.asia-east1.run.app/friends/$_userId'),
        headers: {
        ...
        if (data['status'] == 'success') {
          setState(() {
            ...
            for (var friend in _friendsList) {
              // 處理year_grade字段
              if (friend['year_grade'] != null &&
                  friend['year_grade'].toString().startsWith('G')) {
                final gradeNum = friend['year_grade'].toString().substring(1);
                friend['year_grade'] = '$gradeNum年級';
              }

              // 處理grade字段，以防API同時提供或將來提供這個字段
              if (friend['grade'] != null &&
                  friend['grade'].toString().startsWith('G')) {
  ...
  Future<void> _loadFriendsOnlineStatus() async {
    ...
    try {
      final friendIds =
          _friendsList.map((friend) => friend['user_id'] as String).toList();

      final response = await http.post(
        Uri.parse(
            'https://superb-backend-1041765261654.asia-east1.run.app/online/batch_status'),
          ...
          setState(() {
            ...
            for (String userId in friendIds) {
              if (data['users_status'].containsKey(userId)) {
                _onlineStatus[userId] =
                    data['users_status'][userId]['is_online'] ?? false;
              }
            }
          });
        }
  ...
  Future<void> _loadPendingRequests() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
            'https://superb-backend-1041765261654.asia-east1.run.app/friends/requests/$_userId'),
        headers: {
          'Content-Type': 'application/json; charset=utf-8',
          'Accept': 'application/json; charset=utf-8',
        },
          ...
          setState(() {
            ...
            for (var i = 0; i < _pendingRequests.length; i++) {
              ...
              if (request['requester_grade'] != null &&
                ...
                String grade =
                    request['requester_grade'].toString().substring(1);
                request['requester_grade'] = '$grade年級';
              }

              print(
                  '處理後的請求 #$i: ${request['id']}, ${request['requester_name']}, 年級: ${request['requester_grade']}');
            }
          });
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _searchFriends(String query) async {
    ...
    try {
      final response = await http.post(
        Uri.parse(
            'https://superb-backend-1041765261654.asia-east1.run.app/friends/search'),
        headers: {
          'Content-Type': 'application/json; charset=utf-8',
          'Accept': 'application/json; charset=utf-8',
        },
        ...
        if (data['status'] == 'success') {
          setState(() {
            ...
            for (var user in _searchResults) {
              print(
                  '用戶 ID: ${user['user_id']}, 姓名: ${user['name']}, 好友狀態: ${user['friend_status']}');
  ...
  Future<void> _sendFriendRequest(String friendId) async {
    ...
    try {
      final response = await http.post(
        Uri.parse(
            'https://superb-backend-1041765261654.asia-east1.run.app/friends/send_request'),
        headers: {
          'Content-Type': 'application/json; charset=utf-8',
          'Accept': 'application/json; charset=utf-8',
        },
  ...
  Future<void> _cancelFriendRequest(String requestId, String userId) async {
    ...
    try {
      ...
      if (requestId.isNotEmpty) {
        final response = await http.post(
          Uri.parse(
              'https://superb-backend-1041765261654.asia-east1.run.app/friends/respond_request'),
          headers: {
            'Content-Type': 'application/json; charset=utf-8',
            'Accept': 'application/json; charset=utf-8',
          },
      ...
      final cancelByUserResponse = await http.post(
        Uri.parse(
            'https://superb-backend-1041765261654.asia-east1.run.app/friends/cancel_request'),
        headers: {
          'Content-Type': 'application/json; charset=utf-8',
          'Accept': 'application/json; charset=utf-8',
        },
          ...
          child: Container(
            ...
            child: ListTile(
              ...
              subtitle: Column(
                ...
                children: [
                  if (friend['introduction'] != null &&
                      friend['introduction'].toString().isNotEmpty)
                    Padding(
                  ...
                  if (friend['year_grade'] != null &&
                      friend['year_grade'].toString().isNotEmpty)
                    Padding(
                      padding: EdgeInsets.only(top: 4),
                      child: Text(
          ...
          child: Container(
            ...
            child: ListTile(
              ...
              leading: CircleAvatar(
                radius: 25,
                backgroundImage: request['requester_photo'] != null &&
              ...
              subtitle: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  if (request['requester_intro'] != null &&
                      request['requester_intro'].toString().isNotEmpty)
                    Padding(
                      padding: EdgeInsets.only(top: 4),
                      child: Text(
                  ...
                  if (request['requester_grade'] != null &&
                      request['requester_grade'].toString().isNotEmpty)
                    Padding(
                      padding: EdgeInsets.only(top: 4),
                      child: Text(
              ...
              : _searchResults.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final bool isFriend = _friendsList.any(
                            ...
                            child: ListTile(
                              ...
                              subtitle: Column(
                                ...
                                children: [
                                  ...
                                  if (user['year_grade'] != null &&
                                      user['year_grade'].toString().isNotEmpty)
                                    Padding(
                                      padding: EdgeInsets.only(top: 4),
                                      child: Text(
                                              ...
                                              child: InkWell(
                                                onTap: () {
                                                  ...
                                                  final requestId =
                                                      _getRequestId(user);
                                                  print(
                                                      '收回好友請求點擊 - 請求ID: $requestId, 用戶ID: ${user['user_id']}');
